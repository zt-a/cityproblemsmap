...
- ✅ Карточки проблем с автоматической сменой фото
...
### 1. Редактирование проблем (для автора) 🔧
**Приоритет: ВЫСОКИЙ**

**API готово:**
- `PATCH /api/v1/problems/{entity_id}` — обновить проблему
- Можно менять: title, description, category, media, location

**Что нужно сделать:**
- Кнопка "Редактировать" в ProblemDetail (только для автора)
- Форма редактирования (переиспользовать ProblemCreateForm)
- Показать историю изменений в timeline
- Валидация прав доступа

**Файлы:**
- `src/components/problem/ProblemEditForm.tsx` (создать)
- `src/components/problem/ProblemDetail.tsx` (добавить кнопку)
- `src/hooks/useProblems.ts` (добавить useUpdateProblem)

---

### 2. Публичные профили пользователей 👤
**Приоритет: ВЫСОКИЙ**

**API готово:**
- `GET /api/v1/social/profile/{user_id}` — получить профиль
- `PATCH /api/v1/social/profile` — обновить свой профиль

**Что показывать:**
- Аватар, био, website, social links
- Репутация (reputation score)
- Статистика: followers/following/problems_count
- Список проблем пользователя
- Кнопка "Подписаться" (если не свой профиль)

**Что нужно сделать:**
- `src/pages/UserProfilePage.tsx` — страница профиля
- `src/components/user/UserProfileCard.tsx` — карточка профиля
- `src/components/user/UserStats.tsx` — статистика
- `src/hooks/useUser.ts` — хуки для профилей
- Роут `/users/:userId`

**Интеграция:**
- Кликабельные имена авторов в ProblemCard, ProblemDetail, CommentItem
- Показывать аватары вместо иконок

---

### 3. Social функционал (подписки) 🤝
**Приоритет: ВЫСОКИЙ**

**API готово:**
- `POST /api/v1/social/follow/{user_id}` — подписаться
- `DELETE /api/v1/social/follow/{user_id}` — отписаться
- `GET /api/v1/social/feed` — лента активности подписок

**Что нужно сделать:**
- Кнопка "Подписаться/Отписаться" в профиле
- Страница "Подписки" и "Подписчики"
- Лента активности (feed) — что делают те, на кого подписан
- `src/pages/ActivityFeedPage.tsx`
- `src/components/social/FollowButton.tsx`
- `src/hooks/useSocial.ts`

**Activity types:**
- created_problem, commented, voted, resolved_problem

---

### 4. Прозрачность данных (история версий) 📜
**Приоритет: СРЕДНИЙ**
...
**Что показывать:**
- Полная история изменений проблемы (title, description, status, scores)
- Кто, когда, что изменил
- Diff между версиями (опционально)
- Timeline уже есть, но показывает только базовые события
